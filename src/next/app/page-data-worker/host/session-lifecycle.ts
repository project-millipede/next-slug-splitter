...
import { createWorkerHostLifecycleSession } from '../../../shared/worker/host-lifecycle/session';
...
export type AppPageDataWorkerSession =
  WorkerHostLifecycleSession<AppPageDataWorkerResponse>;

type AppPageDataWorkerSessionRegistry =
  WorkerSessionRegistry<AppPageDataWorkerSession>;
 ...
 */
const appPageDataWorkerHostLifecycleMachine = createWorkerHostLifecycleMachine<
  AppPageDataWorkerResponse,
...
const createAppPageDataWorkerSession = ({
  ...
  const session = createWorkerHostLifecycleSession(
    createWorkerSession<AppPageDataWorkerResponse>({
      sessionKey,
      child
    })
  );
  ...
  child.on('error', error => {
    ...
    appPageDataWorkerHostLifecycleMachine.observeSessionTermination({
      workerSessions,
      session,
      rejectionError: createWorkerExitError({
        workerLabel: 'App page-data worker',
