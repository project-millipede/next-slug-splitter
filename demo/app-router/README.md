...
- `generated-handlers/interactive/page.tsx` bundles only `ExamplePreview`
- `generated-handlers/dashboard/page.tsx` bundles only `FlowComposer` + `ComponentWorkbench`
...
├── interactive/page.tsx   ← imports only ExamplePreview
└── dashboard/page.tsx     ← imports only FlowComposer + ComponentWorkbench
```
...
│   ├── interactive.mdx                 ← heavy (uses <ExamplePreview />)
│   └── dashboard.mdx                   ← heavy (uses <FlowComposer />, <ComponentWorkbench />)
├── lib
...
The shared demo components and ballast generator live in
`packages/ballast-kit`.
